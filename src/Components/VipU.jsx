...
export default function VipU({ index, openIndex, setOpenIndex }) {
  const isOpen = openIndex === index;

  const electricityWorks = [
    ["تأسيس أسلاك سويدي أو سعودي درجة أولي", "خرطوم و علب ماجيك و فليكسبل مصطفي محمود", "لوحه تأسيس 18خط من نوع فينوس ", "قواطع السويدي ( هيميل )"],
    ...
    ["تأسيس كشاف طوارئ في الريسبشن ", "تأسيس ليد مغناطيس في الريسبشن ", "تأسيس كشاف طوارئ في الريسبشن ", "تأسيس ليد مغناطيس في الريسبشن ", "تأسيس كشافات مقاومه للماء ف الريسبشن و الطرق ", "تاسيس كشافات سقف ديكور لتابلوهات "],
  ...
  const sections = [
    ...
    { title: "الديكور", items: ["عمل جنب قطيفه في حدود 5 متر او استوكا انتيكا في حدود 5 متر", "جانب بديل خشب لا يتعدي 3 متر", "عمل بانو جبس في الريسبشن و الطرقه", "عمل خلفيه 3d  في اوضه الاطفال على حسب طلب العميل", "جانب بديل رخام لا يتعدي 3 متر "] },
      ...
      <div className="flex flex-col px-5 md:px-16">
        <div className="flex items-center justify-between cursor-pointer" onClick={() => setOpenIndex(isOpen ? null : index)}>
          <div className="text-white flex items-center space-x-1.5 text-left text-base md:text-[25px] font-semibold">
            ...
            <p>باقة</p>
            <p>Vip_U</p>
            <span className="text-[#21B7A6] text-xs md:text-[22px] pr-2.5">5500 جينيه</span>
          </div>
      ...
      {isOpen && (
        <div className="px-10 mt-5 space-y-6">
          ...
          <div className="mt-5 flex flex-col lg:flex-row lg:space-x-3 space-y-2 lg:space-y-0">
            <ItemWithIcon icon={verify} text="تشطيب السباكه قاعده و حوض بحد اقصي 10000 جنية للحمام الواحد بحد اقصي 2 حمام " />
            <ItemWithIcon icon={verify} text="تشطيب السباكه قاعده و حوض بحد اقصي 10000 جنية للحمام الواحد بحد اقصي 2 حمام " />
